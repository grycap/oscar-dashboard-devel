      ...
      if(index === pathSegments.length-3){
        return (
          <React.Fragment key={currentPath}>
            <Slash size={12} className="pt-[2px] text-slate-400" aria-hidden="true" />
        ...
        return (
          <React.Fragment key={currentPath}>
            ...
            <Link
              ...
              className="no-underline hover:text-slate-900 hover:underline transition-colors duration-200 text-slate-700 font-medium"
            >
              {segment}
            </Link>
          </React.Fragment>
    ...
    customHeader={
      !isOnRoot ? (
        <Link
          ...
          className="text-muted-foreground no-underline hover:text-slate-800 transition-colors duration-200 flex items-center gap-2"
    ...
    secondaryRow={ 
      !isOnRoot ? ( 
        <div className="flex flex-col w-full ">
          <nav className="grid grid-cols-[auto_1fr_auto] my-[6px] mx-1 items-center text-sm gap-0 border rounded-lg border-slate-300 bg-slate-50 shadow-sm overflow-hidden" aria-label="Breadcrumb">
            <Link
              to={`/ui/minio/${name}`}
              className="font-bold no-underline hover:text-slate-900 hover:bg-slate-100 px-0 py-2 transition-all duration-200 border-r border-slate-300 bg-white"
              aria-label={`Navigate to bucket ${name}`}
            >
              <div className="flex flex-row items-center px-4 gap-2">
                <FolderRoot size={19} className="text-slate-600"></FolderRoot>
                <span className="text-slate-900 font-semibold">{name}</span>
              </div>
            </Link>
            <div className="flex flex-row items-center px-3 py-2 bg-gradient-to-r from-slate-50 to-slate-100">
              <div className="flex flex-row items-center text-slate-600 gap-1">
                {breadcrumbs.length > 0 && breadcrumbs ? breadcrumbs : <Slash size={12} className="pt-[2px] text-slate-400" aria-hidden="true" />}
              </div>
            </div>
            <Button variant="outline" size="sm" onClick={() => setRefreshBucketItems(!refreshBucketItems)} title="Refresh" className="group text-slate-600 hover:text-slate-800 hover:bg-slate-200 border-y-0 border-r-0 border-l border-l-slate-300 rounded-none" >
      ...
      <div className="flex flex-row items-center w-full justify-end gap-2">
        ...
        <div className="flex flex-row items-center w-full justify-between gap-4" >
          <div className="flex flex-col">
            <Link
              to={`/ui/minio/${name}`}
              className="font-bold text-black no-underline hover:text-slate-700 transition-colors duration-200 min-w-max"
              aria-label={`Navigate to bucket ${name}`}
            >
              <div className="flex flex-row items-center text-[16px]">
                {name}
              </div>
              <span className={`text-slate-600 text-[14px] flex flex-row uppercase ${isLoading ? 'blur-[3px] animate-pulse' : ''}`}>
                {bucket.visibility ? bucket.visibility : Bucket_visibility.private}
              </span>
            </Link>
            <div className={`grid grid-cols-[auto_auto] items-center font-bold text-slate-500 text-[13px] text-nowrap -mt-1 gap-2 ${isLoading ? 'blur-[3px] animate-pulse' : ''}`}>
              {/* Owner Name */}
              <div 
              ...
              {bucket.metadata?.from_service &&
                <Link 
                  ...
                  className="grid grid-cols-[auto_1fr] no-underline hover:underline underline-offset-2 border-l border-slate-400 pl-2"
                >
